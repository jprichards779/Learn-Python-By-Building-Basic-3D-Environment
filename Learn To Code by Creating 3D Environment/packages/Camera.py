...
class Camera:
    ...
    def output(self):
        ...
        self.points = self.projection(x1,y1,z1, front_view = False, plan = False)
    ...
    def projection(self, x,y,z, front_view = False, plan = False):
        if self.data in self.Camera_dict:
            ...
            if plan and not front_view:         
                ...
                X_axis = [round(self.size*X_array[n]*self.zoom/VectorArray[n], 
                        10) for n in range(len(X_array))]
                Y_axis = [round(-self.size*Y_array[n]*self.zoom/VectorArray[n], 
                        10) for n in range(len(X_array))]
                ...
                self.size = 5
                VectorArray = [1 for i in X_array]
                X_axis = [round(0.5*self.size*X_array[n]*self.zoom/VectorArray[n], 
                        10) for n in range(len(X_array))]
                Y_axis = [round(0.5*self.size*Z_array[n]*self.zoom/VectorArray[n], 
                        10) for n in range(len(X_array))]
                points = pygame_array(X_axis, Y_axis, WIDTH, HEIGHT)
                self.points = points
